(function(root){
  ...
  // 3.2.1 map-reduce真值检测

  // map是映射的作用 将各个元素，按照一定的规则，逐个映射为新的元素。这是个一一对应的过程

  // reduce是个规约的作用,一个规约过程仍然需要按照迭代指定列表的每个元素，然后仍然按照一定规则，合并这些元素到一个目标对象上，这是一个由多至一的过程，或者说是一个逐步积累的过程

  // createReduce接收一个参数 dir这个参数是让reduce从下标0向右运行 或者是让recude从下标最后一项像左执行
  var createReduce = function(dir) {

    var reduce = function(obj, iteratee, memo, init) {
      // 如果obj不是一个数组的话 那么默认按照对象来处理
      // Object.keys是获取对象的属性名并且放进一个数组里面
      var keys = !_.isArray(obj) && Object.keys(obj),
          // 如果keys存在的话 获取keys的长度 如果keys不存在获取obj的长度
          length = (keys || obj).length,
          // 如果dir大于0的话 说明是从下标为0的向右运行
          // 如果dir小于0的话 说明是从下标最后一项往前运行
          index = dir > 0 ? 0 : length - 1;
      // 当init为false的时候
      if (!init) {
        // 当init为false的时候 说明 用户只传了小于3个的参数 那么memo自然而然的就是一个undefined
        // 因为memo是一个累加的值 我们要给memo一个初始的值 这个初始的值就是从被整合对象的第一项为一个memo 或者最后一项
        memo = obj[keys ? keys[index]: index];
        // 然后为了排除这一项 我们要给index进行+1 或者 -1
        index += dir;
      }
      // 为了整合的时候 不会报错 所以我们的index得大于等于0并且不能大于这个数组的长度
      // 每次index+=dir 也就是index +1 或者-1
      for (; index >= 0 && index < length; index += dir) {
        // 获取当前的下标 如果keys存在的话 那就拿取相对应的每一项的属性名
        // 如果不存在直接把index复制给他
        var currentKey = keys ? keys[index] : index;
        // 迭代器的第一个参数就是每次累加的值 
        // 第二个参数 就是当前的值
    ...
    // 当调用_.reduce的时候 他会调用createReduce方法， 最后返回出来的就是这个函数
    return function(obj, iteratee, memo, context) {
      // 当所传过来的实参个数大于或等于3的话 那么就把true传给init
      // 如果没有的话 那么就把false传给init
      var init = arguments.length >= 3;
      // 然后返回reduce的执行结果
      // 这里的obj就是我们所传过去的要被整合的数组对象
      // optimizeCb(iteratee, context, 4) 是最终的迭代器 张这样
      // function(memo, value, index, obj) {
      //   return func.call(context, memo, value, index, obj);
      // };
      // memo是一个累加的结果
      // init 就是一个布尔值
  ...
  // 其实_.reduce的方法就是通过一个creatReduce的工厂函数创建出来的
  _.reduce = createReduce(1);


  // 真值检测其实就是将所有符合条件的放到一个数组里面 并且返回出去
  _.filter = _.select = function(obj, predicate, context) {
    var results = [];
    // 通过cb将predicate这个迭代器进行一下优化 并且将上下文的指定对象为context
    predicate = cb(predicate, context);
    // 然后通过遍历的方式 将所有符合条件的值放进results里面
